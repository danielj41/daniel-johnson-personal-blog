...
title: "3 ways to autofocus a text input in React that ALMOST always work!"
...
There's one situation where I find that `autoFocus` doesn't work. Recall that I mentioned earlier how we render React components into legacy parts of our app:
...
What if we wanted to manually control when `focus()` gets called on the `input` DOM element? We can do that with _refs_ in React. Refs allow us to access the actual DOM elements that React creates.
...
The `[]` dependency list in the `useEffect` call looks a bit suspicious. That means the effect will only run on component mount, and never again.

Thus, if you display the `input` element _after_ the component mounts, we won't focus it. In this example, imagine `loading` is `true` for the first 5 seconds.

```jsx
const AutofocusFunctionComponent = ({ loading }) => {
  const inputElement = useRef(null);
...
There are other ways to work around this problem. For example, you could split the `input` into another component, `AutofocusInput`. Then when `AutofocusInput` mounts, it will always be visible, since the loading is kept in the parent component.

In the meantime, I'm going to move on to _callback refs_.
...
<input
  ref={element => {
    // save element on `inputElement.current` so that we can access it later
...
In this example, our autofocusing will work with the loading indicator situation we described above. It works since `callbackRef` will get called after the `input` element is created.
...
I would recommend that you use the `autoFocus` prop, unless you have odd circumstances that prevent it from working.

Otherwise, I'm hoping that this comprehensive overview gives you the understanding you need to pick the correct solution for your specific app.
